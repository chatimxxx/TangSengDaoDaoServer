...
(
    1000
) NOT NULL DEFAULT '' COMMENT '用户简介',
    avatar_url VARCHAR
(
    1000
) NOT NULL DEFAULT '' COMMENT '用户头像 URL',
    blog VARCHAR
(
    1000
) NOT NULL DEFAULT '' COMMENT '用户博客 URL',
    events_url VARCHAR
(
    1000
) NOT NULL DEFAULT '' COMMENT '用户事件 URL',
    followers INT NOT NULL DEFAULT 0 COMMENT '用户粉丝数',
    followers_url VARCHAR
(
    1000
) NOT NULL DEFAULT '' COMMENT '用户粉丝 URL',
    following INT NOT NULL DEFAULT 0 COMMENT '用户关注数',
    following_url VARCHAR
(
    1000
) NOT NULL DEFAULT '' COMMENT '用户关注 URL',
    gists_url VARCHAR
(
    1000
) NOT NULL DEFAULT '' COMMENT '用户 Gist URL',
    html_url VARCHAR
(
    1000
) NOT NULL DEFAULT '' COMMENT '用户主页 URL',
    member_role VARCHAR
(
    100
) NOT NULL DEFAULT '' COMMENT '用户角色',
    organizations_url VARCHAR
(
    1000
) NOT NULL DEFAULT '' COMMENT '用户组织 URL',
...
(
    1000
) NOT NULL DEFAULT '' COMMENT '用户接收事件 URL',
    remark VARCHAR
(
    100
) NOT NULL DEFAULT '' COMMENT '企业备注名',
    repos_url VARCHAR
(
    1000
) NOT NULL DEFAULT '' COMMENT '用户仓库 URL',
    stared INT NOT NULL DEFAULT 0 COMMENT '用户收藏数',
    starred_url VARCHAR
(
    1000
) NOT NULL DEFAULT '' COMMENT '用户收藏 URL',
    subscriptions_url VARCHAR
(
    1000
) NOT NULL DEFAULT '' COMMENT '用户订阅 URL',
    url VARCHAR
(
    1000
) NOT NULL DEFAULT '' COMMENT '用户 URL',
    watched INT NOT NULL DEFAULT 0 COMMENT '用户关注的仓库数',
    weibo VARCHAR
(
    1000
) NOT NULL DEFAULT '' COMMENT '用户微博 URL',
    `type` VARCHAR
(
    20
) NOT NULL DEFAULT '' COMMENT '用户类型',
    `gitee_created_at` VARCHAR
(
    30
) NOT NULL DEFAULT '' COMMENT 'gitee用户创建时间',
    `gitee_updated_at` VARCHAR
(
    30
) NOT NULL DEFAULT '' COMMENT 'gitee用户更新时间',
...
ALTER TABLE `user`
    ADD COLUMN gitee_uid VARCHAR(100) NOT NULL DEFAULT '' COMMENT 'gitee的用户id';
...
(
    255
) NOT NULL COMMENT '节点ID',
    avatar_url VARCHAR
(
    1000
) NOT NULL COMMENT '头像URL',
    gravatar_id VARCHAR
(
    1000
) NOT NULL COMMENT 'Gravatar ID',
    url VARCHAR
(
    1000
) NOT NULL COMMENT 'GitHub URL',
    html_url VARCHAR
(
    1000
) NOT NULL COMMENT 'GitHub HTML URL',
    followers_url VARCHAR
(
    1000
) NOT NULL COMMENT '关注者URL',
    following_url VARCHAR
(
    1000
) NOT NULL COMMENT '被关注者URL',
    gists_url VARCHAR
(
    1000
) NOT NULL COMMENT '代码片段URL',
    starred_url VARCHAR
(
    1000
) NOT NULL COMMENT '收藏URL',
    subscriptions_url VARCHAR
(
    1000
) NOT NULL COMMENT '订阅URL',
    organizations_url VARCHAR
(
    1000
) NOT NULL COMMENT '组织URL',
    repos_url VARCHAR
(
    1000
) NOT NULL COMMENT '仓库URL',
    events_url VARCHAR
(
    1000
) NOT NULL COMMENT '事件URL',
    received_events_url VARCHAR
(
    1000
) NOT NULL COMMENT '接收事件URL',
    `type` VARCHAR
(
    100
...
(
    100
) NOT NULL DEFAULT '' COMMENT '博客',
    location VARCHAR
(
    255
) NOT NULL DEFAULT '' COMMENT '所在地',
    email VARCHAR
(
    100
) NOT NULL DEFAULT '' COMMENT '电子邮件',
...
(
    100
) NOT NULL DEFAULT '' COMMENT 'Twitter 用户名',
...
ALTER TABLE `user`
    ADD COLUMN github_uid VARCHAR(100) NOT NULL DEFAULT '' COMMENT 'github的用户id';
