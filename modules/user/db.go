...
func (d *DB) QueryByKeyword(keyword string) (*Model, error) {
	var m Model
	err := d.db.Table("user").Where("(short_no=? and short_no<>'') or (username=? and username<>'') or (phone=? and phone<>'') ", keyword, keyword, keyword).First(&m).Error
	return &m, err
}
...
func (d *DB) QueryByUsername(username string) (*Model, error) {
	var m Model
	err := d.db.Table("user").Where("username=? or concat(zone,phone)=? or email=?", username, username, username).First(&m).Error
	return &m, err
}
...
func (d *DB) QueryUIDsByUsernames(usernames []string) ([]string, error) {
	var uids []string
	err := d.db.Select("uid").Table("user").Where("username in ?", usernames).Find(&uids).Error
	return uids, err
}
...
func (d *DB) QueryByPhone(zone string, phone string) (*Model, error) {
	var m Model
	err := d.db.Table("user").Where("zone=? and phone=?", zone, phone).First(&m).Error
	return &m, err
}
...
func (d *DB) QueryByPhones(phones []string) ([]*Model, error) {
	var ms []*Model
	err := d.db.Table("user").Where("CONCAT(`zone`,`phone`) in ?", phones).Find(&ms).Error
	return ms, err
}

// Insert 添加用户
func (d *DB) Insert(m *Model) error {
	err := d.db.Table("user").Create(m).Error
	return err
}
...
func (d *DB) QueryByUID(uid string) (*Model, error) {
	var m Model
	err := d.db.Table("user").Where("uid=?", uid).First(&m).Error
	if err != nil {
		return nil, err
	}
	return &m, err
...
func (d *DB) QueryByVercode(vercode string) (*Model, error) {
	var m Model
	err := d.db.Table("user").Where("vercode=?", vercode).First(&m).Error
	return &m, err
}
...
func (d *DB) queryByQRVerCode(QRVercode string) (*Model, error) {
	var m Model
	err := d.db.Table("user").Where("qr_vercode=?", QRVercode).First(&m).Error
	return &m, err
}

func (d *DB) queryByUIDs(uids []string) ([]*Model, error) {
	var ms []*Model
	err := d.db.Table("user").Where("uid in ?", uids).Find(&ms).Error
	return ms, err
}
func (d *DB) queryAll() ([]*Model, error) {
	var ms []*Model
	err := d.db.Table("user").Where("is_destroy=0 and bench_no='' ").Find(&ms).Error
	return ms, err
}
...
func (d *DB) QueryDetailByUID(uid string, loginUID string) (*Detail, error) {
	var m Detail
	err := d.db.Select("user.*,IFNULL(user_setting.mute,0) mute,IFNULL(user_setting.top,0) top,IFNULL(user_setting.chat_pwd_on,0) chat_pwd_on,IFNULL(user_setting.revoke_remind,0) revoke_remind,IFNULL(user_setting.screenshot,0) screenshot,IFNULL(user_setting.receipt,0) receipt").Table("user").Joins("user_setting on user.uid=user_setting.to_uid and user_setting.uid=?").Where("user.uid=?", loginUID, uid).First(&m).Error
	return &m, err
}
...
func (d *DB) QueryDetailByUIDs(uids []string, loginUID string) ([]*Detail, error) {
	...
	var ms []*Detail
	err := d.db.Select("user.*,IFNULL(user_setting.mute,0) mute,IFNULL(user_setting.top,0) top,IFNULL(user_setting.chat_pwd_on,0) chat_pwd_on,IFNULL(user_setting.revoke_remind,0) revoke_remind,IFNULL(user_setting.screenshot,0) screenshot,IFNULL(user_setting.receipt,0) receipt").Table("user").Joins("user_setting on user.uid=user_setting.to_uid and user_setting.uid=?").Where("user.uid in ?", loginUID, uids).Find(&ms).Error
	return ms, err
}
...
func (d *DB) QueryUserWithOnlyShortNo(shortNo string) (*Model, error) {
	var m Model
	err := d.db.Select("user.name,user.username").Table("user").Where("short_no=?", shortNo).First(&m).Error
	return &m, err
}

// UpdateUsersWithField 修改用户基本资料
func (d *DB) UpdateUsersWithField(field string, value string, uid string) error {
	err := d.db.Table("user").Update(field, value).Where("uid=?", uid).Error
	return err
}

// AddOrRemoveBlacklist 添加黑名单
func (d *DB) AddOrRemoveBlacklistTx(uid string, touid string, blacklist int, version int64, tx *gorm.DB) error {
	err := tx.Table("user_setting").Update("blacklist", blacklist).Update("version", version).Where("uid=? and to_uid=?", uid, touid).Error
	return err
}

// Blacklists  黑名单列表
func (d *DB) Blacklists(uid string) ([]*BlacklistModel, error) {
	var ms []*BlacklistModel
	err := d.db.Select("user.name,user.username,user.uid").Table("user").Joins("user_setting on user.uid=user_setting.to_uid and user_setting.blacklist=1").Where("user_setting.uid=?", uid).Find(&ms).Error
	return ms, err
}
...
func (d *DB) QueryByCategory(category string) ([]*Model, error) {
	var ms []*Model
	err := d.db.Table("user").Where("category=?", category).Find(&ms).Error
	return ms, err
}

func (d *DB) queryWithCategories(categories []string) ([]*Model, error) {
	var ms []*Model
	err := d.db.Table("user").Where("category in ?", categories).Find(&ms).Error
	return ms, err
}
...
func (d *DB) QueryWithAppID(appID string) ([]*Model, error) {
	var ms []*Model
	err := d.db.Table("user").Where("app_id=? and status=1", appID).Find(&ms).Error
	return ms, err
}
...
func (d *DB) queryUserCount() (int64, error) {
	var count int64
	err := d.db.Table("user").Count(&count).Error
	return count, err
}
...
func (d *DB) queryRegisterCountWithDate(date string) (int64, error) {
	var count int64
	err := d.db.Table("user").Where("date_format(created_at,'%Y-%m-%d')=?", date).Count(&count).Error
	return count, err
}
...
func (d *DB) queryRegisterCountWithDateSpace(startDate, endDate string) ([]*Model, error) {
	var ms []*Model
	err := d.db.Table("user").Where("date_format(created_at,'%Y-%m-%d')>=? and date_format(created_at,'%Y-%m-%d')<=?", startDate, endDate).Find(&ms).Error
	return ms, err
}

func (d *DB) updateUser(userMap map[string]interface{}, uid string) error {
	err := d.db.Table("user").Updates(userMap).Where("uid=?", uid).Error
	return err
}

func (d *DB) updatePassword(password string, uid string) error {
	err := d.db.Table("user").Update("password", password).Where("uid=?", uid).Error
	return err
}

// 注销账户
func (d *DB) destroyAccount(uid, username, phone string) error {
	err := d.db.Table("user").Updates(map[string]interface{}{
		"phone":      phone,
		"username":   username,
		"is_destroy": 1,
	}).Where("uid=?", uid).Error
...
func (d *DB) queryWithWXOpenIDAndWxUnionid(wxOpenid, wxUnionid string) (*Model, error) {
	var m Model
	err := d.db.Table("user").Where("wx_openid=? and wx_unionid=?", wxOpenid, wxUnionid).First(&m).Error
	return &m, err
}
...
func (d *DB) queryWithGiteeUID(giteeUID string) (*Model, error) {
	var m Model
	err := d.db.Table("user").Where("gitee_uid=?", giteeUID).First(&m).Error
	return &m, err
}
...
func (d *DB) queryWithGithubUID(githubUID string) (*Model, error) {
	var m Model
	err := d.db.Table("user").Where("github_uid=?", githubUID).First(&m).Error
	return &m, err
}

func (d *DB) updateUserMsgExpireSecond(uid string, msgExpireSecond int64) error {
	err := d.db.Table("user").Update("msg_expire_second", msgExpireSecond).Where("uid=?", uid).Error
	return err
}
func (d *DB) queryUserRedDot(uid, category string) (*userRedDotModel, error) {
	var m userRedDotModel
	err := d.db.Table("user_red_dot").Where("uid=? and category=?", uid, category).First(&m).Error
	return &m, err
}
func (d *DB) insertUserRedDotTx(m *userRedDotModel, tx *gorm.DB) error {
	err := tx.Table("user_red_dot").Create(m).Error
	return err
}

func (d *DB) insertUserRedDot(m *userRedDotModel) error {
	err := d.db.Table("user_red_dot").Create(m).Error
	return err
}
func (d *DB) updateUserRedDot(m *userRedDotModel) error {
	err := d.db.Table("user_red_dot").Updates(map[string]interface{}{
		"count":  m.Count,
		"is_dot": m.IsDot,
	}).Where("uid=? and category=?", m.UID, m.Category).Error
	return err
}

func (d *DB) updateUserRedDotTx(m *userRedDotModel, tx *gorm.DB) error {
	err := tx.Table("user_red_dot").Updates(map[string]interface{}{
		"count":  m.Count,
		"is_dot": m.IsDot,
	}).Where("uid=? and category=?", m.UID, m.Category).Error
	return err
